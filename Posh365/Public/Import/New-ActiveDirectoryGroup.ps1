function New-ActiveDirectoryGroup {
    ...
    Process {
        ForEach ($CurGroup in $Group) {
            ...
            $PrimarySMTP = $CurGroup.EmailAddresses -split [regex]::Escape('|') | Where-Object {$_ -cmatch "SMTP:"}
            $PrimarySMTP = $PrimarySMTP.Substring(5)
            Set-ADGroup -Identity $ADGroup.ObjectGUID -Add @{Mail = $PrimarySMTP}
            $Address = $CurGroup.EmailAddresses -split [regex]::Escape('|') | Where-Object $filter
            if ($Domain) {
