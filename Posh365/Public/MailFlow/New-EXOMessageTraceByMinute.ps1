Function New-EXOMessageTraceByMinute {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    New-EXOMessageTraceByMinute -SenderAddress "User@domain.com" -RecipientAddress "recipient@domain.com" -StartDate 15 -FromIP "xx.xx.xx.xx" -FilePath "C:\Output\FileName.csv"
    ...
    if ($StartDate) {
        [DateTime]$StartDate = ((Get-Date).AddMinutes( - $StartDate))
        $StartDate = $StartDate.ToUniversalTime()
    }

    if ($StartDate) {
        [DateTime]$EndDate = ((Get-Date).AddMinutes( - $EndDate))
