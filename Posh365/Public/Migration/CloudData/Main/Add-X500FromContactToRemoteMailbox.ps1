...
function Add-X500FromContactToRemoteMailbox {
    ...
    $MatchingPrimaryCSV = Join-Path -Path $PoshPath -ChildPath ('SourceContact Matching TargetRemoteMailbox_{0}.csv' -f [DateTime]::Now.ToString('yyyy-MM-dd-hhmm'))
    ...
    switch ($YesNo) {
        0 {
            $TargetResult = Join-Path -Path $PoshPath -ChildPath ('Results of adding x500s to Target Remote Mailboxes_{0}.csv' -f [DateTime]::Now.ToString('yyyy-MM-dd-hhmm'))
