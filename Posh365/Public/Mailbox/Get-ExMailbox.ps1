function Get-ExMailbox {
    ...
    .EXAMPLE
    Get-ExMailbox | Export-Csv c:\scripts\All365Mailboxes.csv -notypeinformation -encoding UTF8

    .EXAMPLE
    Get-ExMailbox -ArchivesOnly | Export-Csv c:\scripts\All365Mailboxes.csv -notypeinformation -encoding UTF8

    .EXAMPLE
    '{emailaddresses -like "*contoso.com"}' | Get-ExMailbox | Export-Csv c:\scripts\365Mailboxes.csv -notypeinformation -encoding UTF8

    .EXAMPLE
    '{emailaddresses -like "*contoso.com"}' | Get-ExMailbox -ArchivesOnly | Export-Csv c:\scripts\365Mailboxes.csv -notypeinformation -encoding UTF8

    .EXAMPLE
    '{emailaddresses -like "*contoso.com"}' | Get-ExMailbox -DetailedReport | Export-Csv c:\scripts\365Mailboxes_Detailed.csv -notypeinformation -encoding UTF8
    ...
    process {
        if ($MailboxFilter) {
            foreach ($CurMailboxFilter in $MailboxFilter) {
                if (-not $ArchivesOnly) {
                    Get-Mailbox -Filter $CurMailboxFilter @EXOArchiveMin | Select-Object ($Selectproperties + $CalculatedProps)
                }
                else {
                    Get-Mailbox -Archive -Filter $CurMailboxFilter @EXOArchiveMin | Select-Object ($Selectproperties + $CalculatedProps)
                }
            }
        }
        else {
            if (-not $ArchivesOnly) {
                ...
                $MailboxList = Get-Mailbox @EXOArchiveMin
                ...
                if ($ConfirmCount -eq 'n' -or $ConfirmCount -eq 'y') {
                    ...
                    foreach ($Mailbox in $MailboxList) {
                        ...
                        $Stat = $Mailbox | Get-ExMailboxStatistics
                        if ($Stat) {
                ...
                Get-Mailbox -Archive @EXOArchiveMin | Select-Object ($Selectproperties + $CalculatedProps)
            }
        }
    }
    end {
